...
export default function ReceptionistDashboard({ onLogout, loggedInStaff }) {
  ...
  // Email Gateway Configuration States (Reception Exclusive)
  const [emailJsConfig, setEmailJsConfig] = useState(() => {
    return JSON.parse(localStorage.getItem('dhms_emailjs_config') || '{"serviceId":"","templateId":"","publicKey":""}');
  });
  const [testEmailAddress, setTestEmailAddress] = useState('');
  const [testEmailStatus, setTestEmailStatus] = useState(null);
  const [sentEmailsList, setSentEmailsList] = useState(() => {
    return JSON.parse(localStorage.getItem('dhms_sent_emails') || '[]');
  });
...
==================================================`;
  ...
  const isEmailConfigured = Boolean(emailJsConfig?.serviceId && emailJsConfig?.templateId && emailJsConfig?.publicKey);

  const handleSaveEmailConfig = (e) => {
    e.preventDefault();
    localStorage.setItem('dhms_emailjs_config', JSON.stringify(emailJsConfig));
    alert("Hospital Email Gateway credentials saved successfully!");
  };

  const handleClearEmailConfig = () => {
    if (window.confirm("Are you sure you want to clear the Email Gateway configuration?")) {
      const reset = { serviceId: '', templateId: '', publicKey: '' };
      setEmailJsConfig(reset);
      localStorage.removeItem('dhms_emailjs_config');
      alert("Email Gateway configuration cleared.");
    }
  };

  const handleSendTestLiveEmail = async (e) => {
    e.preventDefault();
    ...
    if (!isEmailConfigured) {
      alert("Please enter and save your Service ID, Template ID, and Public Key first before sending a test email.");
      return;
    }

    setTestEmailStatus({ loading: true, message: 'Dispatching test live email via EmailJS...' });
    ...
    try {
      ...
      if (res.ok) {
        setTestEmailStatus({
          success: true,
          message: `🎉 Success! Real test email was delivered to ${testEmailAddress}. Please check your inbox (and spam folder).`
        });
      } else {
        const errText = await res.text();
        setTestEmailStatus({
          success: false,
          message: `❌ Delivery Error (${res.status}): ${errText || 'Invalid Service ID, Template ID, or Public Key.'}`
        });
      }
    } catch (err) {
      setTestEmailStatus({
        success: false,
        message: `❌ Network / Dispatch Error: ${err.message}`
      });
    }
  };
  ...
  const renderEmailSettings = () => (
    <div className="rd-view-container">
      <div className="rd-header-banner">
        <div>
          <h2>Hospital Email Gateway Configuration</h2>
          <p>Configure EmailJS to automatically send real emails with login passwords and hospital care messages directly to patients' real inboxes upon registration.</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ 
            ...
            fontSize: '12.5px', 
            fontWeight: '700',
            background: isEmailConfigured ? '#dcfce7' : '#fef3c7',
            color: isEmailConfigured ? '#15803d' : '#b45309',
            border: isEmailConfigured ? '1px solid #86efac' : '1px solid #fde68a'
          }}>
            {isEmailConfigured ? '🟢 Live Email Gateway Active' : '🟡 Simulated Dispatch Mode'}
          </span>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '20px', marginTop: '20px' }}>
        ...
        <div className="rd-card" style={{ padding: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <h3 style={{ margin: 0, fontSize: '17px', color: '#1e293b' }}>⚙️ EmailJS Credentials</h3>
            <a 
              href="https://dashboard.emailjs.com/admin" 
              target="_blank" 
              rel="noreferrer"
              style={{ fontSize: '12.5px', color: '#2563eb', fontWeight: '600', textDecoration: 'none' }}
            >
              Open EmailJS Dashboard ↗
            </a>
          </div>

          <form onSubmit={handleSaveEmailConfig} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div className="rd-form-group">
              ...
              />
              <span style={{ fontSize: '11px', color: '#64748b' }}>From Email Services tab in EmailJS (connected to your Gmail or Outlook).</span>
            </div>
            ...
            <div className="rd-form-group">
              ...
              <input 
                type="text" 
                required 
                placeholder="e.g. template_welcome_patient"
                value={emailJsConfig.templateId}
                onChange={e => setEmailJsConfig({ ...emailJsConfig, templateId: e.target.value })}
                style={{ width: '100%', padding: '10px 12px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '13.5px', boxSizing: 'border-box' }}
              />
              <span style={{ fontSize: '11px', color: '#64748b' }}>From Email Templates tab in EmailJS.</span>
            </div>

            <div className="rd-form-group">
              <label style={{ fontWeight: '600', fontSize: '13px', color: '#334155' }}>
                Public Key (Account API Key) <span style={{ color: 'red' }}>*</span>
              </label>
              <input 
                type="text" 
                required 
                placeholder="e.g. pk_live_xxxx or user_xxxx"
                value={emailJsConfig.publicKey}
                onChange={e => setEmailJsConfig({ ...emailJsConfig, publicKey: e.target.value })}
                style={{ width: '100%', padding: '10px 12px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '13.5px', boxSizing: 'border-box' }}
              />
              <span style={{ fontSize: '11px', color: '#64748b' }}>From Account &gt; API Keys tab in EmailJS.</span>
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
              ...
              {isEmailConfigured && (
                <button 
                  ...
                  style={{ padding: '10px 16px', background: '#fee2e2', border: '1px solid #fca5a5', color: '#b91c1c', borderRadius: '6px', fontWeight: '600', cursor: 'pointer' }}
                >
                  Clear
                </button>
              )}
          ...
          <div style={{ marginTop: '24px', borderTop: '1px solid #e2e8f0', paddingTop: '20px' }}>
            <h4 style={{ margin: '0 0 10px 0', fontSize: '14px', color: '#1e293b' }}>🧪 Send Real Test Email</h4>
            <p style={{ margin: '0 0 12px 0', fontSize: '12px', color: '#64748b' }}>Send a real test welcome email to verify that your connected Gmail/Outlook sends emails properly.</p>
            
            <form onSubmit={handleSendTestLiveEmail} style={{ display: 'flex', gap: '8px' }}>
              <input 
                type="email" 
                placeholder="Enter your email to test (e.g. you@gmail.com)"
              ...
              <button 
                ...
                style={{ background: '#2563eb', color: 'white', border: 'none', borderRadius: '6px', padding: '9px 16px', fontWeight: '700', fontSize: '13px', cursor: 'pointer', whiteSpace: 'nowrap' }}
              >
            ...
            {testEmailStatus && (
              <div style={{ 
                ...
                borderRadius: '8px', 
                fontSize: '13px',
                background: testEmailStatus.success ? '#f0fdf4' : (testEmailStatus.loading ? '#eff6ff' : '#fef2f2'),
                border: testEmailStatus.success ? '1px solid #bbf7d0' : (testEmailStatus.loading ? '1px solid #bfdbfe' : '1px solid #fecaca'),
                color: testEmailStatus.success ? '#15803d' : (testEmailStatus.loading ? '#1d4ed8' : '#b91c1c')
              }}>
        ...
        {/* 2-Minute Setup Guide & Template Parameters */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div className="rd-card" style={{ padding: '22px' }}>
            <h3 style={{ margin: '0 0 12px 0', fontSize: '16px', color: '#1e293b' }}>📖 2-Minute Setup Instructions</h3>
            <ol style={{ margin: 0, paddingLeft: '18px', fontSize: '13px', color: '#475569', lineHeight: 1.6 }}>
              <li>Create a free account at <strong><a href="https://www.emailjs.com" target="_blank" rel="noreferrer" style={{ color: '#2563eb' }}>emailjs.com</a></strong>.</li>
              <li>Go to <strong>Email Services</strong> &gt; Add your Hospital Gmail / Outlook account &gt; Copy <strong>Service ID</strong>.</li>
              <li>Go to <strong>Email Templates</strong> &gt; Create New Template &gt; Copy <strong>Template ID</strong>.</li>
              <li>Go to <strong>Account &gt; API Keys</strong> &gt; Copy <strong>Public Key</strong>.</li>
              <li>Paste the 3 keys here and hit <strong>Save Credentials</strong>!</li>
            </ol>
          </div>

          <div className="rd-card" style={{ padding: '22px' }}>
            <h3 style={{ margin: '0 0 12px 0', fontSize: '15px', color: '#1e293b' }}>📋 Template Variables</h3>
            <p style={{ margin: '0 0 10px 0', fontSize: '12px', color: '#64748b' }}>Include these variable tags in your EmailJS template:</p>
            <table style={{ width: '100%', fontSize: '12px', borderCollapse: 'collapse' }}>
              <tbody>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '6px 0', fontFamily: 'monospace', fontWeight: '700', color: '#2563eb' }}>{'{{to_name}}'}</td>
                  <td style={{ padding: '6px 0', color: '#64748b' }}>Patient's Full Name</td>
                </tr>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '6px 0', fontFamily: 'monospace', fontWeight: '700', color: '#2563eb' }}>{'{{to_email}}'}</td>
                  <td style={{ padding: '6px 0', color: '#64748b' }}>Patient's Email Address</td>
                </tr>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '6px 0', fontFamily: 'monospace', fontWeight: '700', color: '#2563eb' }}>{'{{patient_id}}'}</td>
                  <td style={{ padding: '6px 0', color: '#64748b' }}>Generated Patient Health ID</td>
                </tr>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '6px 0', fontFamily: 'monospace', fontWeight: '700', color: '#2563eb' }}>{'{{password}}'}</td>
                  <td style={{ padding: '6px 0', color: '#64748b' }}>Generated Portal Password</td>
                </tr>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '6px 0', fontFamily: 'monospace', fontWeight: '700', color: '#2563eb' }}>{'{{message}}'}</td>
                  <td style={{ padding: '6px 0', color: '#64748b' }}>Hospital care & thank you message</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div className="rd-card" style={{ marginTop: '24px', padding: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div>
            <h3 style={{ margin: 0, fontSize: '17px', color: '#1e293b' }}>📨 Sent Emails & Credentials Dispatch Log</h3>
            <span style={{ fontSize: '12px', color: '#64748b' }}>History of all welcome emails and access passcodes sent to patients.</span>
          </div>
          <span style={{ background: '#f1f5f9', padding: '4px 10px', borderRadius: '12px', fontSize: '12px', fontWeight: '700', color: '#475569' }}>
            {sentEmailsList.length} Total Emails Dispatched
          </span>
        </div>

        {sentEmailsList.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '30px', color: '#94a3b8', fontSize: '13.5px' }}>
            No emails dispatched yet. When you register a new patient at Reception, their email dispatch record will appear here.
          </div>
        ) : (
          <table className="rd-table" style={{ width: '100%', fontSize: '13px' }}>
            <thead>
              <tr>
                <th>Patient Name</th>
                <th>Recipient Email</th>
                <th>Patient ID</th>
                <th>Dispatched Passcode</th>
                <th>Date & Time</th>
                <th>Delivery Mode</th>
                <th>Status</th>
              </tr>
            ...
            <tbody>
              {sentEmailsList.map(eml => (
                <tr key={eml.id}>
                  <td><strong>{eml.recipientName}</strong></td>
                  <td>{eml.recipientEmail}</td>
                  <td><span style={{ color: '#1e3a8a', fontWeight: '700' }}>{eml.patientId}</span></td>
                  <td><code style={{ background: '#f1f5f9', padding: '2px 6px', borderRadius: '4px', color: '#166534', fontWeight: '700' }}>{eml.password}</code></td>
                  <td>{new Date(eml.sentAt).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}</td>
                  <td>
                    <span style={{ fontSize: '11px', background: eml.deliveryMode?.includes('Live') ? '#dcfce7' : '#eff6ff', color: eml.deliveryMode?.includes('Live') ? '#15803d' : '#1d4ed8', padding: '2px 8px', borderRadius: '4px', fontWeight: '700' }}>
                      {eml.deliveryMode || 'Simulated & Logged'}
                    </span>
                  </td>
                  <td>
                    <span className="rd-status-badge on-duty" style={{ background: '#dcfce7', color: '#15803d' }}>
                      ✓ {eml.status}
                    </span>
                  </td>
                </tr>
      ...
      <header className="rd-topbar">
        ...
        <div className="rd-topbar-right">
          <button 
            ...
            style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', background: isEmailConfigured ? '#f0fdf4' : '#eff6ff', border: isEmailConfigured ? '1px solid #bbf7d0' : '1px solid #bfdbfe', borderRadius: '8px', color: isEmailConfigured ? '#15803d' : '#1d4ed8', fontSize: '12.5px', fontWeight: '700', cursor: 'pointer' }}
          >
            📧 {isEmailConfigured ? 'Live Email Active' : 'Email Gateway'}
          </button>
      ...
      <div className="rd-body">
        ...
        <aside className="rd-sidebar">
          <ul className="rd-nav">
            ...
            <li className={activeTab === 'email_settings' ? 'active' : ''} onClick={() => setActiveTab('email_settings')}>
              ...
              Email Gateway
              {isEmailConfigured && (
                <span style={{ marginLeft: 'auto', background: '#10b981', color: 'white', padding: '1px 6px', borderRadius: '10px', fontSize: '10px', fontWeight: '800' }}>
                  LIVE
                </span>
              )}
            </li>
        ...
        <main className="rd-main">
          ...
          {activeTab === 'email_settings' && renderEmailSettings()}
        </main>
      </div>
